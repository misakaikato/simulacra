...
Three scenarios ship in `examples/` and double as the acceptance fixtures:
...
| `referendum`               | 200 voters loaded from a CSV survey file decide a local levy; an `intervene` step asks 20 of them to advocate, a questionnaire is the ballot, and a plugin metric turns the answers into a support share | `homophily`, `activation`, `feedSize`, `memoryWindow`                                                  | `supportShare`, `stanceAssortativity`                |
